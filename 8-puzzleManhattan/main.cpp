...
// compute the number of steps vertically and horizontally from the correct place
int manhattanHeuristic(string state){
    int result=0;
    
    int manhattanValue[9][9] = { // distance cost to all possiable place
        {0,1,2,1,2,3,2,3,4},
        {1,0,1,2,1,2,3,2,3},
        {2,1,0,3,2,1,4,3,2},
        {1,2,3,0,1,2,1,2,3},
        ...
        if (goalState[i] != state[i]) {
            for (int j=0; j<=goalState.length(); j++) {
                if (goalState[j] == state[i]) {
                    result = result + manhattanValue[j][i];
                }
            }
        }
    }
...
void aStarManhattan(string inFileName){
    ...
    infile.close();
    int manhattan = 0,gValue=0;
    manhattan = manhattanHeuristic(eightPuzzle); // initially f = 0 + h
    stateToOpen(eightPuzzle,gValue,manhattan,NULL); // put initial state into open list
    string curState=""; //current state
    int curGValue=0, fValue=0;
    ...
    while (!(openList->get_listHead()->getNext() == NULL)) {
        ...
        manhattan = manhattanHeuristic(curState);
        
        if (manhattan==0) { // if node is the goal, we are done
            ...
            if (emptyTileIndex !=2 && emptyTileIndex != 5 && emptyTileIndex !=8) {
                ...
                manhattan = manhattanHeuristic(nextState);
                gValue = curGValue + 1;
                fValue = gValue + manhattan;
...
int main(int argc, const char * argv[]) {
    ...
    double duration;
    //aStarManhattan(easy);
    //aStarManhattan(medium);
    //aStarManhattan(hard);
    aStarManhattan(worst);
